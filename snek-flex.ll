...
// snek-flex.ll
//  
// This is an implementation of a lexical analysis tool for the Snek
...
// snek-flex.cc.
//
// The code relies on snek-flex.hh which defines the class named
// Snek::Lexer. This is the wrapper class for the lexer, and is a
...
// file `snek-bison.tab.hh` generated by Bison.
//
// ------
//
...
// a series of method definitions for the Snek Lexer class.
//
    ...
    #include "snek-bison.tab.hh"       // Defines Snek::Parser::token
                                       //     and Snek::Parser::PLUS, etc.
    //
    using token = Snek::Parser::token;                 // Useful aliases.
    using location_type = Snek::Parser::location_type;

    // The method declared below is generated by Flex.
    //
    #undef  YY_DECL
    #define YY_DECL int Snek::Lexer::yylex(Snek::Parser::semantic_type* const lval, location_type* loc)
    ...
    // class Snek::Lexer
    //
    // Provides a sequence of Snek tokens to be parsed.
    //
    ...
    //
    void Snek::Lexer::advance_by_text(std::string txt, location_type* l) {
        l->step();
        for (char c: txt) {
            advance_by_char(c,l);
        }
    ...
    //
    void Snek::Lexer::advance_by_char(char curr_char, location_type* l) {
        if (curr_char == '\n' ) {
            l->lines();
        } else if (curr_char == '\t') {
    ...
    //
    int Snek::Lexer::indent_column(std::string txt) {
        int spaces = 0;
        for (char c: txt) {
            if (c == '\t') {
                spaces += 8 - spaces % 8;
    ...
    //
    Locn Snek::Lexer::locate(const location_type &l) {
        return Locn { src_name, l.begin.line, l.begin.column };
    }

    // lx.bail(m)
    //
    // Throws a SnekError exception with the message `m`.
    //
    void Snek::Lexer::bail(Snek::Parser::location_type* l,
                              std::string msg) {
